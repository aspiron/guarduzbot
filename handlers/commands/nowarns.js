...
const nowarnsHandler = async ({ message, reply, state, telegram }) => {
	...
	if (!userToUnwarn) {
		return reply(
			'ℹ️ <b>Userga javob qaytaring yoki uni xabarda eslating.</b>',
			replyOptions
	...
	if (warns.length === 0) {
		return reply(
			`ℹ️ ${link(userToUnwarn)} <b> userda ogohlantirishlar yo'q.</b>`,
			replyOptions
		);
	}
	...
	return reply(
		`♻️ ${link(user)} adminimiz ${link(userToUnwarn)}ning ` +
		'<b>barcha ogohlantirishlarini kechirdi.</b>',
		replyOptions
	);
};
