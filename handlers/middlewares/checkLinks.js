...
const dh = {
	blacklistedDomain: R.always(Action.Warn('Taqiqlangan domenga link berildi')),
	...
	tme: async url => {
		...
		if (url.searchParams.has('start')) return Action.Warn('Bot referal link');
		...
		return Action.Warn('Telegram guruh yoki kanalga link berildi');
	},
};
...
const classifyAsync = memoize(async url => {
	...
	if (blacklisted.protocol(url)) return Action.Warn('Tg protokolidan foydalanib link berildi');
...
module.exports = async (ctx, next) =>
	(await classifyCtx(ctx)).cata({
		...
		Notify(error) {
			...
			if (notifyBrokenLink) {
				ctx.reply(
					`️ℹ️ Link ${error.url} ishlamayabdi (${error.code}).`,
					{ reply_to_message_id }
				);
			}
			return next();
